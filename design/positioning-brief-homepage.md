...
- *The homepage never shows the engine doing anything.* The animated graph is decorative, not demonstrative — no schema line, no traversal, no "two agents on one memory" moment. The most concrete asset (the worked Domain Explorations) is buried in a generic "Start here" card.
...
- *Can I actually use it now?* The hero CTA "Quickstart" implies self-serve; the Engine page reveals it's **preview access by request** ("source and local run instructions are shared directly with approved collaborators"). That mismatch breaks trust with exactly the engineer the page is for.
...
- *Is it open source?* "Open Engine," "Open by design," "no rug-pull" are asserted, but no license, no repo, and gated access. Openness is claimed, not demonstrated.
...
**What is this?**
The Open Engine is a typed, governable knowledge graph that gives your AI agents a shared memory they can reason over — and that you own and export in full.

**Why should I care?**
Agents, chats, and tools keep forgetting context and disagreeing with each other; the Open Engine turns that scattered context into one durable, queryable model of your domain that every tool can build on instead of re-explaining from scratch.

**What can I do with it this week?**
Request preview access, model one real domain in a readable schema, load your first nodes, and run a traversal your agents can reason over — with full-fidelity export from day one.

> Note on the "this week" line: it is written to match the **current gated reality** (preview-by-request), not a public download. If the engine becomes self-serve before launch, swap to "Pull it, model a domain, and run your first traversal tonight." See §8 honesty constraints.
...
- **Subhead:** The Open Engine is a typed knowledge graph that turns scattered context into a living model your AI can reason over. Model your domain once; let Claude, Codex, and MCP-capable tools build on the same graph — and export everything, anytime.
...
- **Subhead:** The Open Engine is a typed, governable knowledge graph — a durable model of your domain that agents query instead of guessing from text, and that you own and export in full.
...
- **Subhead:** The Open Engine is the typed substrate under agentic systems: meaning that persists across tools, structure your models can traverse, and full ownership with no lock-in.
...
Replace the three risk-reversal ticks (`No lock-in / Full-fidelity export / Governed when you graduate`) with a **capability strip** that says what the engine *does*, and demote risk-reversal to one quiet line.
...
1. **Hero** — capability promise (§3A) + a demonstrative graph/schema snippet (§4). Primary CTA *Request preview access*, secondary *See a worked model*.
...
3. **What you actually do** — reframe the flywheel as a concrete 3–4 step path: *Model a domain → Connect your agents → Query & reason → Export anytime.* Make step 1 honest (request access), not "run tonight."
...
**Sequencing recommendation:** if the preview already exposes an MCP/agent interface to external collaborators, build it as its own page and link it from the hero chip. If only export formats are live today, ship it first as a **"Connect" section on the Engine page** (framed roadmap-honest), and promote it to a standalone page once the interface is publicly demonstrable. Either way, this is the page that makes the homepage hero's harness-agnostic promise land.
...
4. **"Graph database" / leading with "database."** Accurate-adjacent but drops the engine into a crowded, commoditized category (Neo4j et al.) and throws away the agent-memory + governance differentiation.
5. **"Run it tonight" / "Quickstart" while access is gated.** Sets an expectation the next click breaks. Either change the product (publish openly) or change the verb to match reality. Don't promise a download that isn't there.
...
- Hero CTAs: `Quickstart` → `Request preview access` (mailto already wired, line 166); change secondary to `See a worked model →` pointing to `domain-explorations.html`.
...
- **Access is preview-by-request, not a public download.** `engine.html` (lines 118–139) is the source of truth: "source and local run instructions are shared directly with approved collaborators." Hero copy and the "this week" action must not imply self-serve download until that changes.
