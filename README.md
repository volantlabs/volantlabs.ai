...
- **Perspectives publishing model:** public copy presents authored essays and field notes. The JSON generator still supports provenance metadata for future graph-drafted work; keep that future-state language out of public copy until the workflow is real. New posts start as JSON in `content/perspectives/`; run `node scripts/build-perspectives.mjs` to regenerate article pages in `perspectives/`, `assets/perspectives-data.js`, `feed.xml`, and the generated blocks in `index.html` / `perspectives.html`. A source with `publicationState: "preview"` generates a direct, `noindex` editorial-preview page but stays out of public listings, feeds, sitemaps, and discovery indexes until a graph `PublishingDecision` authorizes publication. Before promoting or exporting a revision, refresh `content/perspectives.publishing-decisions.json` from `volant_base`; exports expire after 24 hours. The generator fails closed unless the referenced decision is the latest non-expired approval for the same Perspective, at publish/update scope, made by the named reviewer, evidenced by the declared assessment, newer than the source revision, and bound to its deterministic content hash. Use `node scripts/build-perspectives.mjs --check` before committing.
...
- **Social previews:** `scripts/generate-social-cards.mjs` is the source of truth for Open Graph copy, typography, brand mode, output filename, and `assets/images/social/manifest.json`. Generated background inputs live in `assets/images/social/backgrounds/`. Engine and Community are Vellis-led with a Volant Labs publisher endorsement; the remaining cards are Volant Labs-led. Run `npm run social-cards` after changing a background, logo, or card definition. Use a new versioned PNG filename for a visual revision so social-platform caches receive a new URL, and retain previously published PNGs.
- **Repo scripts:** `npm run social-cards` regenerates social preview PNGs and their manifest; `npm run build` regenerates Perspectives outputs; `npm run check` verifies generated files are current.
